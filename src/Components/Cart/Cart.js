...
const Cart = ({ addCart, setAddCart }) => {
  ...
  //   const newTotalPrice = newQuantity * 10;
  //   setTotalPrice(newTotalPrice);
  // };

  const handleRemove = (params) => {
    const item = JSON.parse(localStorage.getItem("addCart"));
    //  console.log(item)
    const remove = addCart.filter((items) => items.id !== params);
    setAddCart(remove);
  };
      ...
      <div className="cart-whole">
        <div className="cart_left">
          ...
          {addCart && addCart.length > 0
            ? addCart.map((item, id) => {
                return (
                  <table className="table_cart" key={id}>
                    <tr className="row_table">
                      ...
                      <th>
                        ...
                        <button className="tooltip" onClick={() => handleRemove(item.id)}>
                          <svg
        ...
        <div className="cart_right">
          <p className="title_cart">Billing</p>
